...
//        update on the patient search functionality - enhancing the search speed - Issue#reg10
    ...
    public List<Encounter> getDeadPatientsForEhr(EncounterType encounterType, Date startDate, Date endDate) throws DAOException {
        ...
        /*if(startDate == null && endDate == null) {
            criteria.add(Restrictions.and(Restrictions.ge("encounterDatetime", DateUtils.getStartOfDay(new Date())),
                    Restrictions.le("encounterDatetime", DateUtils.getEndOfDay(new Date()))));
        }*/
        if(startDate != null) {
            criteria.add(Restrictions.ge("encounterDatetime", DateUtils.getStartOfDay(startDate)));
        }
    ...
    public EhrMorgueQueue saveEhrMorgueQueue(EhrMorgueQueue ehrMorgueQueue) throws DAOException {
        sessionFactory.getCurrentSession().saveOrUpdate(ehrMorgueQueue);
        return ehrMorgueQueue;
    }
    ...
    public EhrMorgueStrength saveEhrMorgueStrength(EhrMorgueStrength ehrMorgueStrength) throws DAOException {
        sessionFactory.getCurrentSession().saveOrUpdate(ehrMorgueStrength);
    ...
    public AppointmentServiceType saveAppointmentServiceType(AppointmentServiceType appointmentServiceType) throws DAOException {
        sessionFactory.getCurrentSession().saveOrUpdate(appointmentServiceType);
    ...
    public MigrationTracking getMigrationPatientTrackingDetailsByOldPatientId(Integer oldPatientId) throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(MigrationTracking.class);
        criteria.add(Restrictions.eq("oldPatientId", oldPatientId));
        return (MigrationTracking) criteria.uniqueResult();
    }

    @Override
    public MigrationVisitsTracking getMigrationVisitsTrackingDetailsByOldVisitId(Integer oldVisitId) throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(MigrationVisitsTracking.class);
        criteria.add(Restrictions.eq("oldVisitId", oldVisitId));
        return (MigrationVisitsTracking) criteria.uniqueResult();
    }
