...
public class HibernateEhrAppointmentBlockDAO extends HibernateEhrSingleClassDAO implements EhrAppointmentBlockDAO {

    public HibernateEhrAppointmentBlockDAO() {
        super(EhrAppointmentBlock.class);
    }
    ...
    public List<EhrAppointmentBlock> getEhrAppointmentBlocks(Date fromDate, Date toDate, String locations, Provider provider,
                                                          List<EhrAppointmentType> appointmentTypes) {
        List<EhrAppointmentBlock> filteredAppointmentBlocks = null;
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(EhrAppointmentBlock.class);
        ...
        if (appointmentTypes != null && appointmentTypes.size() > 0) {
            ...
            for (EhrAppointmentType type : appointmentTypes) {
        ...
        List<EhrAppointmentBlock> appointmentBlocks = criteria.list();
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EhrAppointmentBlock> getOverlappingEhrAppointmentBlocks(EhrAppointmentBlock appointmentBlock) {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(EhrAppointmentBlock.class);
        Disjunction disjunction = Restrictions.disjunction();
        ...
        return new ArrayList<EhrAppointmentBlock>();
    }
}
