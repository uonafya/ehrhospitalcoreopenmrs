	...
	*/
	public List<PatientSearch> searchListOfPatient(Date date, String searchKey,
		...
		String hql = "SELECT DISTINCT ps from PatientSearch ps,OpdTestOrder o INNER JOIN o.patient p where p.patientId=ps.patientId AND o.scheduleDate BETWEEN '"
				...
				+ endDate
				//+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.billableService is NOT NULL GROUP BY o.patient) AND (ps.identifier LIKE '%"
				+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.serviceType=0 AND o.billableService is NOT NULL AND o.valueCoded NOT IN (SELECT c.answerConcept FROM ConceptAnswer c,ConceptName cn WHERE cn.name='MAJOR OPERATION' AND c.concept=cn.concept) AND (ps.identifier LIKE '%"
				+ searchKey + "%' OR ps.fullname LIKE '" + searchKey + "%')";
		int firstResult = (page - 1) * BillingConstants.PAGESIZE;
		Session session = sessionFactory.getCurrentSession();
		Query q = session.createQuery(hql);
	...
	public List<BillableService> listOfServiceOrder(Integer patientId,
		...
		String hql = "from BillableService b where b.conceptId IN (SELECT o.valueCoded FROM OpdTestOrder o where o.patient='"
				...
				+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.serviceType=0 AND o.billableService is NOT NULL AND o.valueCoded NOT IN (SELECT c.answerConcept FROM ConceptAnswer c,ConceptName cn WHERE cn.name='MAJOR OPERATION' AND c.concept=cn.concept))";
		Session session = sessionFactory.getCurrentSession();
		Query q = session.createQuery(hql);
	...
	public List<OpdTestOrder> listOfOrder(Integer patientId, Date date)
		...
		String hql = "from OpdTestOrder o where o.patient='"
				...
				+ "' AND '"
				+ endDate
				+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.serviceType=0 AND o.billableService is NOT NULL AND o.valueCoded NOT IN (SELECT c.answerConcept FROM ConceptAnswer c,ConceptName cn WHERE cn.name='MAJOR OPERATION' AND c.concept=cn.concept) GROUP BY encounter";
		Session session = sessionFactory.getCurrentSession();
		Query q = session.createQuery(hql);
   ...
   public List<WaiverType> getWaiverTypes() throws DAOException {
	...
	public List<PatientSearch> searchListOfMorguePatient(Date date, String searchKey,
		...
		String hql = "SELECT DISTINCT ps from PatientSearch ps,OpdTestOrder o INNER JOIN o.patient p where p.patientId=ps.patientId AND o.scheduleDate BETWEEN '"
				...
				+ endDate
				//+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.billableService is NOT NULL GROUP BY o.patient) AND (ps.identifier LIKE '%"
				+ "' AND o.billingStatus=0 AND o.cancelStatus=0 AND o.serviceType=99 AND o.billableService is NOT NULL AND o.valueCoded NOT IN (SELECT c.answerConcept FROM ConceptAnswer c,ConceptName cn WHERE cn.name='MAJOR OPERATION' AND c.concept=cn.concept) AND (ps.identifier LIKE '%"
				+ searchKey + "%' OR ps.fullname LIKE '" + searchKey + "%')";
		int firstResult = (page - 1) * BillingConstants.PAGESIZE;
		Session session = sessionFactory.getCurrentSession();
		Query q = session.createQuery(hql);
