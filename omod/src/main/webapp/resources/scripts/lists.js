...
function moveAllById( fromListId, targetListId ) {
    jQuery('#'+targetListId).append(jQuery('#'+fromListId+' >option'));
}
...
jQuery(document).ready(function() {
    jQuery("select").each(function(i,o){
...
function filterList( filter, listId ) {
    ...
    jQuery("#"+listId+" option").filter(function(i) {
    ...
    jQuery("#"+listId+"_storage option").filter(function(i) {
        var toMatch = jQuery(this).text().toString().toLowerCase();
        var filterLower = filter.toString().toLowerCase();
        return toMatch.indexOf(filterLower) != -1;
    }).appendTo("#"+listId);
    var sortedVals = jQuery.makeArray(jQuery("#"+listId+" option")).sort(function(a,b){
        return jQuery(a).text() > jQuery(b).text() ? 1: -1;
    });
    jQuery("#"+listId).empty().html(sortedVals);
    jQuery("#"+listId).val('--');

    /* For FF only - no performance issues
    jQuery("#" + listId).find('option').hide();
    jQuery("#" + listId).find('option:Contains("' + filter + '")').show();*/
}
