...
<script type="text/javascript">
	/**
	 * June 4th 2012: Thai Chuong add lastDayOfVisist
	...
	PATIENTSEARCH = {
		...
		fromClause : "",
		whereClause : "",
		//ghanshyam 8-april-2013 Support #1300 [Registration]Slowness in the PUNJAB hospitalcore(added groupClause)
		groupClause: "",
		orderClause : "",
		limitClause : "",
		query : "",
		...
		init : function(options) {
			...
			jQuery("#nameOrIdentifier", this.form).keyup(
					function(event) {
						if (event.keyCode == 13) {
							nameInCapital = StringUtils.capitalize(jQuery(
									"#nameOrIdentifier", PATIENTSEARCH.form)
									.val());
							jQuery("#nameOrIdentifier", PATIENTSEARCH.form)
									.val(nameInCapital);
		...
		search : function(newQuery) {
			...
			if (validatedResult) {
				...
				jQuery(PATIENTSEARCH.target)
						.mask(
								"<img src='" + openmrsContextPath + "/moduleResources/hospitalcore/ajax-loader.gif" + "'/>&nbsp;");

				jQuery.ajax({
					type : "POST",
					url : openmrsContextPath
							+ "/module/hospitalcore/searchPatient.form",
					data : ({
						query : query,
					...
					success : function(data) {
						...
						if (PATIENTSEARCH.currentRow == 0) {
							...
							jQuery(PATIENTSEARCH.target).append(
									"<div>"
											+ PATIENTSEARCH
													.generateNavigation()
											+ "</div>");
		...
		getPatientResultCount : function() {
			...
			jQuery.ajax({
				type : "POST",
				url : openmrsContextPath
						+ "/module/hospitalcore/getPatientResultCount.form",
				data : ({
					query : query
				}),
				success : function(data) {
					...
					jQuery(PATIENTSEARCH.target).append(
							"<div>" + PATIENTSEARCH.generateNavigation()
									+ "</div>");
					// callback
					PATIENTSEARCH.success({
						totalRow : PATIENTSEARCH.totalRow
		...
		buildQuery : function() {
			...
			// Get value from form			
			nameOrIdentifier = jQuery.trim(jQuery("#nameOrIdentifier",
					this.form).val());
			nameOrIdentifier = nameOrIdentifier.replace(/\s/g, "");

			// Build essential query
			//ghanshyam 8-april-2013 Support #1300 [Registration]Slowness in the PUNJAB hospitalcore(commented old query below and written new query after this commented query)
			/*
			...
			this.whereClause += " (pi.identifier LIKE '%"
					+ nameOrIdentifier
		...
		buildAgeQuery : function() {
			...
			if (value != undefined && value.length > 0) {
				...
				range = parseInt(jQuery.trim(jQuery("#ageRange", this.form)
						.val()));

				if (type == "y") {

					jQuery("#rangeUnit").html("Year(s)");
					this.whereClause += "AND (EXTRACT(YEAR FROM (FROM_DAYS(DATEDIFF(NOW(),ps.birthdate)))) >="
							+ (value - range)
					...
					this.whereClause += "AND (DATEDIFF(NOW(),ps.birthdate) <= "
							+ (days) + ") ";
				}
			}
		},
		...
		buildRelativeNameQuery : function() {
			...
			if (value != undefined && value.length > 0) {
			    //ghanshyam 8-april-2013 Support #1300 [Registration]Slowness in the PUNJAB hospitalcore
				...
				this.whereClause += " AND (patRelativeName.name LIKE '%"
						+ personAttributeTypeName
						+ "%' AND paRelativeName.value LIKE '%" + value + "%')";
			}
		},
		...
		buildPhoneNumberQuery : function() {
			...
			if (value != undefined && value.length > 0) {
				...
				this.whereClause += " AND (patPhoneNumber.name LIKE '%"
						+ phoneNumberAttributeTypeName
						+ "%' AND paPhoneNumber.value LIKE '%" + value + "%')";
			}
		},

		/** June 4th 2012: Thai Chuong added for lastDayOfVisit
			Supported issue #248*/
		...
		buildLastVisitQuery : function() {
			...
			if (value != 'Any') {
				...
				this.whereClause += " AND (DATEDIFF(NOW(), e.date_created) <= "
						+ value + ")";
			}
		},
		...
		validate : function() {
			jQuery("#errorSection", this.form).html(
		...
		validateNameOrIdentifier : function() {
			...
			if (value.length >= 3) {
				...
				for (i = 0; i < value.length; i++) {
					if (pattern.indexOf(value[i]) < 0) {
						jQuery("#errorList", this.form)
								.append(
										"<li>Please enter patient name/identifier in correct format!</li>");
						return false;
					}
				}
				return true;
				...
				jQuery("#errorList", this.form)
						.append(
								"<li>Please enter at least 3 letters of patient name/identifier</li>");
				return false;
			}
		},
		...
		validateAge : function() {
			if (this.advanceSearch) {
				...
				for (i = 0; i < value.length; i++) {
					if (pattern.indexOf(value[i]) < 0) {
						jQuery("#errorList", this.form).append(
								"<li>Please enter patient age in digits!</li>");
						return false;
					}
				}
				return true;
			...
			if (this.advanceSearch) {
				...
				for (i = 0; i < value.length; i++) {
					if (pattern.indexOf(value[i]) < 0) {
						jQuery("#errorList", this.form)
								.append(
										"<li>Please enter phone number in digits!</li>");
						return false;
					}
				}
				return true;
